...
echo Install Debian
debootstrap --arch=amd64 --variant=minbase stable $TEMP/live_boot/chroot http://mirror/debian/
...
chown -v root:root $TEMP/live_boot/chroot/etc/systemd/network/99-dhcp-en.network
chmod -v 644 $TEMP/live_boot/chroot/etc/systemd/network/99-dhcp-en.network
...
cp -v $TEMP/live_boot/chroot/boot/vmlinuz-* $TEMP/live_boot/staging/live/vmlinuz
cp -v $TEMP/live_boot/chroot/boot/initrd.img-* $TEMP/live_boot/staging/live/initrd
...
mksquashfs $TEMP/live_boot/chroot $TEMP/live_boot/staging/live/filesystem.squashfs
